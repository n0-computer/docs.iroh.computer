...
By default iroh is configured with several public rate-limited relay servers run by number 0 (two in the US, one in Europe, and one in Asia), free to use for development and testing. To prevent abuse, throughput through public relays is rate-limited. For production workloads, use [Shared Relays](/concepts/relays#shared-relays), a [dedicated relay](/concepts/relays#dedicated-relays), or [self-host your own](/concepts/relays#deploy-your-own-relay).
...
Relays](/concepts/relays#shared-relays) and isolated [Dedicated
Relays](/concepts/relays#dedicated-relays). Uptime SLAs are available on
Enterprise plans. To remove the managed-service dependency, self-host on a
