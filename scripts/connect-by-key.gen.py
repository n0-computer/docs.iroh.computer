...
14s loop. SMIL-driven (<animate>/<animateTransform>), single clock. We avoid
CSS keyframes on purpose: Mintlify serves SVG assets with a
`Content-Security-Policy: default-src 'none'` header, which blocks the inline
<style> element that CSS animation needs, so CSS animation runs in `mint dev`
but is dead on the deployed site. SMIL is not governed by style-src and works
in both. See scripts/README.md.

To change it, edit this script and run:  python3 connect-by-key.gen.py
It writes ../images/how-iroh-works/connect-by-key.svg.
The MDX embeds this as <Frame><img src=... style='width:100%'/>; the
...
OUT_PATH = os.path.normpath(os.path.join(
    HERE, "..", "images", "how-iroh-works", "connect-by-key.svg"))
