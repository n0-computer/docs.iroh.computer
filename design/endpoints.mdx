---
title: "Endpoints"
description: "How an iroh endpoint combines a cryptographic identity with network services — identity, setup, home relay selection, and TLS authentication using raw public keys."
---
...
<Note>
For a higher-level introduction to endpoints, see [Endpoints](/concepts/endpoints) in
"How it works".
</Note>
...
- **Addressing**: It is used as a lookup key for address discovery (see [Address Lookup](/design/address-lookup)) and as a routing key by relay servers.
...
The home relay is the primary rendezvous point where other endpoints will reach this endpoint. It is advertised as part of the endpoint's addressing information (see [Addressing](/design/addressing)) and published via address lookup services (see [Address Lookup](/design/address-lookup)).
...
Once the home relay is selected, the endpoint publishes its addressing information so that remote endpoints can find it. This typically includes the home relay URL and, depending on configuration, direct IP addresses. See [Address Lookup](/design/address-lookup) for details on the publication mechanism.
