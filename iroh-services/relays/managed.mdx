...
If you want any endpoint that has your relay's URL to be able to use it — not just your project's endpoints — you can turn authentication off in your relay's **Settings → Network Settings → Access Control**. Use the **Who may connect** dropdown, then **Save**.

In the dashboard this control is labeled **Private** (authentication on — only your project's API keys may connect) and **Public** (authentication off). "Public" here means no API key is required to connect; it does not make your relay discoverable.
...
Public relays still report their traffic and metrics to your dashboard; they just don't require your API key to connect.
