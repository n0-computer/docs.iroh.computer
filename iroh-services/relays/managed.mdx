...
- **Multi-region**: deploy across available regions for resilience

Multi-cloud, on-premises deployments, and SLAs are available on Enterprise
plans. [Contact us](mailto:support@iroh.computer) to discuss those requirements.
...
We review each request for infrastructure and capacity availability. Depending
on the region, provisioning may be available immediately or may require
additional provider and capacity review. We will confirm availability, expected
provisioning time, deployment details, and pricing before provisioning the
relay.

Custom-region deployments can use a different egress allowance and overage rate
than standard dedicated relays. Your quoted deployment terms will specify the
applicable egress pricing. Only traffic carried by the relay counts as relay
egress; direct peer-to-peer traffic does not.
