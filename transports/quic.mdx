...
transports alongside it. See [Tor](/transports/tor), [Nym](/transports/nym), and
[Bluetooth](/transports/bluetooth) for examples.
...
A real-world example is the [media over QUIC protocol](https://doc.moq.dev/) (MoQ in short): MoQ is used
to transfer live video frames. It uses one QUIC stream for each frame (QUIC
streams are cheap to create)!
