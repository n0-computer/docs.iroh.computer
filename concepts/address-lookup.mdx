...
iroh includes Pkarr publishing and address lookup. Its default configuration
publishes signed records to a Pkarr server and resolves them through DNS or
HTTPS-based Pkarr lookup. See the [DNS Address Lookup guide](/connecting/dns-address-lookup)
for configuration details.

Additional crates provide other address lookup options:

- [mDNS address lookup](/connecting/local-address-lookup) finds endpoints on the local network.
- [DHT address lookup](/connecting/dht-address-lookup) publishes and resolves signed records through the BitTorrent Mainline DHT.
