...
<Note>
The `EndpointId` must always be globally unique, otherwise there would
be two nodes impersonating each other. When using a persisted key
make sure no other endpoint is using the same key at the same time.
</Note>
